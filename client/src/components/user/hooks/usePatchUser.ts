    ...
    (newUserData: User) => patchUserOnServer(newUserData, user),
    {
      ...
      onError: (error, newData, context) => {
        ...
        if (context.previousUserData) {
          ...
          toast({
            title: 'Updating is filed: Restored to previous data!',
            status: 'error',
          });
        }
      },
      onSuccess: (userData: User | null) => {
        // updateUser(userData); do not need to update user due to onMutate
        toast({ title: 'Your data has been updated!', status: 'success' });
      },
      onSettled: () => {
